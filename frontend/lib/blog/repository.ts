...
// server process (build, or first request in a long-running server), not on
// every call. Deliberately not React's cache() here — that API's availability
// depends on the exact React/Next version in use, and this data is static
// build-time content, not per-request state, so a plain module cache is both
...
export function getAllPosts(): BlogPost[] {
  ...
  for (const file of files) {
    ...
    const parsed = blogPostSchema.safeParse(JSON.parse(raw));
    if (!parsed.success) {
