...
// "rising/falling ~X%/year", not a statistical forecast.
...
// Latest known cutoff plus one trend-step, clamped to a valid percentage.
// With only one data point this is just that point's value (no trend to add).
export function getProjectedCutoff(history: MeritHistoryPoint[]): number | null {
  ...
  return Math.max(0, Math.min(100, latest + trend));
}
...
// Groups the flat offering list by program name, preserving insertion order
// of first appearance -- used to render one card per program with each
// campus/session as a sub-row, rather than one row per raw data record.
export function groupByProgram(offerings: ProgramOffering[]): Map<string, ProgramOffering[]> {
  const map = new Map<string, ProgramOffering[]>();
