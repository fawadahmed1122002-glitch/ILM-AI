...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
        ...
        <div className="hidden sm:flex items-center gap-4">
          ...
          {user ? (
            <>
              ...
              <span className="text-sm text-slate-500 dark:text-slate-400 max-w-[140px] truncate">
                {user.full_name}
              </span>
      ...
      {menuOpen && (
        <div className="sm:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-4 space-y-3">
          {user ? (
            <>
            ...
            <>
              ...
              <Link href="/register" onClick={() => setMenuOpen(false)}
                className="block text-center text-sm font-semibold text-white bg-teal-700 hover:bg-teal-600 dark:bg-teal-600 dark:hover:bg-teal-500 px-4 py-2.5 rounded-lg transition-colors no-underline">
                Get Started
              </Link>
            </>
          )}
